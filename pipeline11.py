...
_num_regex = re.compile(r"[-+]?(\d{1,3}(\.\d{3})+|\d+)([.,]\d+)?")  # 1.234,56 / 1234.56 / 1234
...
def _plot_grouped(
    ...
    """
    Plota por H2O_pct = {6,25,35} e também combinado.
    Se yerr_col for None, plota sem barras.
    """
    ...
    if any_curve:
        ...
        plt.xlabel("Power (kW)")
        plt.ylabel(y_label)
        plt.title(f"{title_prefix} (all fuels)")
        plt.grid(True, which="both", linestyle="--", linewidth=0.5)
        plt.legend()
...
def make_plots(out_df: pd.DataFrame) -> None:
    # n_th (%): eixo fixo como você pediu
    ...
    # Consumo (kg/h) com incerteza expandida
    ...
    # Kibox_AI_50_1 por combustível (sem incerteza)
    kibox_col = "KIBOX_AI_50_1"
    if kibox_col in out_df.columns:
        _plot_grouped(
            out_df,
            y_col=kibox_col,
            yerr_col=None,
            title_prefix="KIBOX_AI_50_1 vs Power",
            filename_prefix="kibox_AI_50_1_vs_power",
            y_label="KIBOX_AI_50_1",
            fixed_y=None,
        )
    else:
        # ajuda a localizar o nome real caso tenha variação
        candidates = [c for c in out_df.columns if c.startswith("KIBOX_") and "AI_50_1" in c]
        if candidates:
            print(f"[WARN] Não achei exatamente 'KIBOX_AI_50_1'. Encontrei parecidas: {candidates}")
        else:
            print("[WARN] Não plotei KIBOX_AI_50_1: coluna não existe no excel final.")
