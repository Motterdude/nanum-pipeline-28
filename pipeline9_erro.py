...
def parse_meta(path: Path) -> FileMeta:
    """
    Regras:
      - Kibox: nome termina com _i (antes da extensão) -> source_type=KIBOX
      - LabVIEW: resto -> LABVIEW

    Extrai:
      - load_kw: padrão "40KW" (case-insensitive)
      - etoh/h2o: padrão "E65H35"
    """
    basename = path.stem
...
# -------------------------
# Kibox csv read + summary
# -------------------------
def _read_kibox_csv(path: Path) -> pd.DataFrame:
    """
    Leitura robusta: tenta separador automático e trata BOM.
    """
    # sep=None (engine=python) tenta inferir vírgula/; etc
    df = pd.read_csv(path, sep=None, engine="python", encoding="utf-8-sig")
    df.columns = _normalize_cols(list(df.columns))
    # remove colunas "Unnamed" comuns
    df = df.loc[:, ~pd.Series(df.columns).astype(str).str.startswith("Unnamed").values].copy()
    return df
...
def kibox_mean_row(meta: FileMeta) -> pd.DataFrame:
    """
    Retorna 1 linha por arquivo Kibox com médias de todas as colunas numéricas.
    Colunas recebem prefixo KIBOX_ para evitar conflito.
    """
    df = _read_kibox_csv(meta.path)

    # tenta converter tudo que for possível para numérico
    num = df.apply(pd.to_numeric, errors="coerce")

    # pega apenas colunas com pelo menos algum número
    keep_cols = [c for c in num.columns if num[c].notna().any()]
    if not keep_cols:
        # Kibox veio todo texto; devolve só chaves
        out = pd.DataFrame([{
            "Load_kW": meta.load_kw,
            "EtOH_pct": meta.etoh_pct,
            "H2O_pct": meta.h2o_pct,
    ...
    means = num[keep_cols].mean(numeric_only=True)

    row = {f"KIBOX_{k}": float(means[k]) if pd.notna(means[k]) else pd.NA for k in means.index}
    row.update({
        "Load_kW": meta.load_kw,
        "EtOH_pct": meta.etoh_pct,
        "H2O_pct": meta.h2o_pct,
...
def kibox_aggregate(metas: List[FileMeta]) -> pd.DataFrame:
    """
    Agrega múltiplos arquivos Kibox por (Load_kW, EtOH_pct, H2O_pct).
    Se houver mais de um arquivo por condição, faz média das médias.
    """
    rows: List[pd.DataFrame] = []
    for m in metas:
        # só CSV Kibox
        if m.source_type != "KIBOX":
            continue
        if m.path.suffix.lower() != ".csv":
            continue
        if m.load_kw is None or m.etoh_pct is None or m.h2o_pct is None:
            print(f"[WARN] Kibox sem metadados completos (kw/E/H) no nome: {m.path.name}")
            continue
        try:
            rows.append(kibox_mean_row(m))
        except Exception as e:
            print(f"[ERROR] Falha lendo Kibox {m.path.name}: {e}")
    ...
    value_cols = [c for c in allk.columns if c.startswith("KIBOX_") and c != "KIBOX_File"]

    # média das médias por condição
    agg = allk.groupby(key_cols, dropna=False, sort=True)[value_cols].mean(numeric_only=True).reset_index()

    # opcional: contar quantos arquivos entraram
...
# -------------------------
def build_final_table(ponto: pd.DataFrame, lhv: pd.DataFrame, kibox: pd.DataFrame, mappings: dict, instruments: dict) -> pd.DataFrame:
    df = ponto.merge(lhv, on=["EtOH_pct", "H2O_pct"], how="left")

    # merge Kibox mean columns by condition
    if kibox is not None and not kibox.empty:
        df = df.merge(kibox, on=["Load_kW", "EtOH_pct", "H2O_pct"], how="left")
...
def main() -> None:
    ...
    if not kibox_agg.empty:
        print(f"[OK] Kibox agregado: {len(kibox_agg)} condições, {kibox_agg.shape[1]} colunas (incluindo chaves).")
