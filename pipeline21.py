...
def build_final_table(ponto: pd.DataFrame, lhv: pd.DataFrame, kibox_agg: pd.DataFrame, mappings: dict, instruments: dict) -> pd.DataFrame:
    ...
    # =========================
    # absolute humidity + cp mix + Q_EVAP_NET (kW) + NEW engineered delta-T
    # =========================
    ...
    # NEW: engineered intake thermal delta (Tout - Tin)
    # Column name engineered for clarity; delta in °C is numerically equal to K.
    ...
    # Q_EVAP_NET (kW): net enthalpy change proxy of the intake flow between sensors
    if "Air_kg_h" in df.columns and t_adm_col and "T_E_CIL_AVG_mean_of_windows" in df.columns:
        ...
        df["Q_EVAP_NET_kW"] = mdot_air * cp_used * dT  # (kg/s)*(kJ/kgK)*K = kJ/s = kW
    else:
...
def make_plots_all(out_df: pd.DataFrame) -> None:
    ...
    # NEW: plot relationship between T_ADMISSÃO and T_E_CIL_AVG (XY, all fuels)
    t_adm_col = _find_first_col_by_substrings(out_df, ["t", "admiss"])
    if t_adm_col and "T_E_CIL_AVG_mean_of_windows" in out_df.columns:
        plot_all_fuels_xy(
            out_df,
            t_adm_col,
            "T_E_CIL_AVG_mean_of_windows",
            None,
            f"T_E_CIL_AVG vs {t_adm_col} (all fuels)",
            f"t_e_cil_avg_vs_{_safe_name(t_adm_col)}_all.png",
            "T_ADMISSÃO (°C)",
            "T_E_CIL_AVG (°C)",
            None,
        )
        ...
        if not t_adm_col:
            print("[WARN] Não plotei T_E_CIL_AVG vs T_ADMISSÃO: não achei coluna de T_ADMISSÃO no output.")
        elif "T_E_CIL_AVG_mean_of_windows" not in out_df.columns:
            print("[WARN] Não plotei T_E_CIL_AVG vs T_ADMISSÃO: falta T_E_CIL_AVG_mean_of_windows no output.")
