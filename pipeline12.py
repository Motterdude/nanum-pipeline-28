...
def make_plots(out_df: pd.DataFrame) -> None:
    ...
    kibox_col = "KIBOX_AI50_AVG_1"
    if kibox_col in out_df.columns:
        _plot_grouped(
            out_df,
            y_col=kibox_col,
            yerr_col=None,
            title_prefix="KIBOX_AI50_AVG_1 vs Power",
            filename_prefix="kibox_AI50_AVG_1_vs_power",
            y_label="KIBOX_AI50_AVG_1",
            fixed_y=None,
        )
    else:
        candidates = [c for c in out_df.columns if c.startswith("KIBOX_") and "AI50" in c]
        print(f"[WARN] Não achei '{kibox_col}'. Candidatos parecidos: {candidates[:20]}")
