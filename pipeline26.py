...
def load_config_excel() -> Tuple[dict, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    # Plots sheet (rev3). Optional. If missing => empty DF => fall back to make_plots_all().
...
def make_plots_from_config(out_df: pd.DataFrame, plots_df: pd.DataFrame) -> None:
    ...
    Config-driven plotting:
      - Each row in plots_df defines one plot.
      - No need to edit code to add a new plot: just add a new row with x_col/y_col names.
    Supported plot_type:
      - all_fuels_yx      -> uses plot_all_fuels (x_col optional; default Load_kW)
      - all_fuels_xy      -> uses plot_all_fuels_xy
      - all_fuels_labels  -> uses plot_all_fuels_with_value_labels
    Histogram is intentionally not included here (per user request).
    """
    if plots_df is None or plots_df.empty:
        print("[WARN] Plots config vazio; usei make_plots_all (default hardcoded).")
        make_plots_all(out_df)
        return
    ...
    for _, r in plots_df.iterrows():
        ...
        plot_type = str(r.get("plot_type", "")).strip()
        filename = str(r.get("filename", "")).strip()
        title = str(r.get("title", "")).strip()

        if not plot_type or not filename:
            print(f"[WARN] Plots row inválida (plot_type/filename vazios). Pulei. title='{title}'")
            continue

        x_col_req = str(r.get("x_col", "")).strip()
        y_col_req = str(r.get("y_col", "")).strip()
        yerr_req = str(r.get("yerr_col", "")).strip()

        x_label = str(r.get("x_label", "")).strip()
        y_label = str(r.get("y_label", "")).strip()

        x_min = r.get("x_min", pd.NA)
        x_max = r.get("x_max", pd.NA)
        x_step = r.get("x_step", pd.NA)
        y_min = r.get("y_min", pd.NA)
        y_max = r.get("y_max", pd.NA)
        y_step = r.get("y_step", pd.NA)

        fixed_x = _parse_axis_spec(x_min, x_max, x_step)
        fixed_y = _parse_axis_spec(y_min, y_max, y_step)

        fuels = _parse_csv_list_ints(r.get("filter_h2o_list", pd.NA))
        fuels_override = fuels if fuels is not None else None

        label_variant = str(r.get("label_variant", "box")).strip().lower() or "box"

        # Resolve columns (case-insensitive) only if provided
        try:
            x_col = resolve_col(out_df, x_col_req) if x_col_req else "Load_kW"
        except Exception as e:
            print(f"[WARN] Plot '{filename}': x_col '{x_col_req}' não encontrado. Pulei. ({e})")
            continue

        if not y_col_req:
            print(f"[WARN] Plot '{filename}': y_col vazio. Pulei.")
            continue

        try:
            y_col = resolve_col(out_df, y_col_req)
        except Exception as e:
            print(f"[WARN] Plot '{filename}': y_col '{y_col_req}' não encontrado. Pulei. ({e})")
            continue
        ...
        if yerr_req:
            try:
                ...
                yerr_col = None
                print(f"[WARN] Plot '{filename}': yerr_col '{yerr_req}' não encontrado. Vou plotar sem erro.")
        ...
        # Dispatch
        if plot_type.lower() in {"all_fuels_yx", "all_fuels", "all_fuels_y_vs_x"}:
            plot_all_fuels(
                out_df,
                y_col=y_col,
                yerr_col=yerr_col,
        ...
        elif plot_type.lower() in {"all_fuels_xy", "xy"}:
            plot_all_fuels_xy(
                out_df,
                x_col=x_col,
                y_col=y_col,
        ...
        elif plot_type.lower() in {"all_fuels_labels", "labels"}:
            ...
            print(f"[WARN] Plot '{filename}': plot_type '{plot_type}' não suportado. Pulei.")


def make_plots_all(out_df: pd.DataFrame) -> None:
    if "n_th_pct" in out_df.columns and "U_n_th_pct" in out_df.columns:
        plot_all_fuels(out_df, "n_th_pct", "U_n_th_pct", "n_th vs Power (all fuels)", "nth_vs_power_all.png", "Thermal efficiency (%)", (0, 42, 2), (0, 55, 5), "Load_kW", "Power (kW)", None)
    else:
    ...
    if cons_col in out_df.columns and "U_Consumo_kg_h" in out_df.columns:
        plot_all_fuels(out_df, cons_col, "U_Consumo_kg_h", "Fuel consumption vs Power (all fuels)", "consumo_vs_power_all.png", "Fuel consumption (kg/h)", None, (0, 55, 5), "Load_kW", "Power (kW)", None)
    else:
        print("[WARN] Não plotei consumo: faltam colunas Consumo_kg_h_mean_of_windows / U_Consumo_kg_h")

    if "MFB_10_90" in out_df.columns:
        plot_all_fuels(out_df, "MFB_10_90", None, "MFB_10_90 vs Power (all fuels)", "mfb_10_90_vs_power_all.png", "MFB_10_90 (degCA)", None, (0, 55, 5), "Load_kW", "Power (kW)", None)
    ...
    if "Air_kg_h" in out_df.columns:
        plot_all_fuels(out_df, "Air_kg_h", None, "Air mass flow vs Power (all fuels)", "air_kg_h_vs_power_all.png", "Air (kg/h)", None, (0, 55, 5), "Load_kW", "Power (kW)", None)

    if "Air_g_s" in out_df.columns:
        plot_all_fuels(out_df, "Air_g_s", None, "Air mass flow vs Power (all fuels)", "air_g_s_vs_power_all.png", "Air (g/s)", None, (0, 55, 5), "Load_kW", "Power (kW)", None)

    if "UMIDADE_ABS_g_m3" in out_df.columns:
        plot_all_fuels(out_df, "UMIDADE_ABS_g_m3", None, "Absolute humidity vs Power (all fuels)", "umidade_abs_g_m3_vs_power_all.png", "Absolute humidity (g/m³)", None, (0, 55, 5), "Load_kW", "Power (kW)", None)

    if "T_E_CIL_AVG_mean_of_windows" in out_df.columns:
        plot_all_fuels(out_df, "T_E_CIL_AVG_mean_of_windows", None, "T_E_CIL_AVG vs Power (all fuels)", "t_e_cil_avg_vs_power_all.png", "Temperature (°C)", None, (0, 55, 5), "Load_kW", "Power (kW)", None)

    if "Q_EVAP_NET_kW" in out_df.columns:
        plot_all_fuels(out_df, "Q_EVAP_NET_kW", None, "Q_EVAP_NET vs Power (all fuels)", "q_evap_net_kw_vs_power_all.png", "Q_EVAP_NET (kW)", None, (0, 55, 5), "Load_kW", "Power (kW)", None)
    ...
    for c in sorted(kibox_cols):
        plot_all_fuels(out_df, c, None, f"{c} vs Power (all fuels)", f"kibox_{_safe_name(c)}_vs_power_all.png", c, None, (0, 55, 5), "Load_kW", "Power (kW)", None)
...
def main() -> None:
    ...
    # Plots driven by config (rev3). If sheet missing/empty => fall back to current hardcoded plotting.
    make_plots_from_config(out, plots_df)

    # Histograms intentionally left out of the Plots-config workflow for now (per request)
    if kibox_files:
        print("[INFO] Kibox csv em raw/ detectado, mas histogramas KPEAK estão fora do workflow de plots-config por enquanto.")
    else:
