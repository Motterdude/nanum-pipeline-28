...
def _get_resolution_for_key(instruments_df: pd.DataFrame, key_norm: str) -> Optional[float]:
    """
    Returns a representative resolution for a logical key, if present.
    Uses the max(abs(resolution)) across matching rows (conservative).
    """
...
def uB_from_instruments_rev2(x: pd.Series, key_norm: str, instruments_df: pd.DataFrame) -> pd.Series:
    ...
    RULE: if the key does NOT exist in Instruments, return NA (do not compute).
    """
    if instruments_df is None or instruments_df.empty:
...
def build_final_table(
    ...
    # Power workflow (will produce NA if no power_kw in Instruments)
    ...
    # Fuel consumption workflow (instrument-based part may be NA; balance-based uB handled later)
    df = _add_uncertainty_workflow_cols(
        df=df,
        key_norm="fuel_kgh",
    ...
    # Use balance-based uB from compute_ponto_stats (already aggregated)
    df["uB_Consumo_kg_h"] = pd.to_numeric(df.get("uB_Consumo_kg_h_mean_of_windows", pd.NA), errors="coerce")

    # Keep instrument-derived (if any) without breaking your current balance-based pipeline
    df["uB_Consumo_kg_h_instrument"] = df["uB_Consumo_kg_h"]

    # Recompute combined/expanded for fuel using balance-based uB (may be NA if no balance_kg instrument)
    ...
    # =========================
    # Temperature uncertainty workflow for T_ADMISSÃO
    # RULE: only if Instruments contains key 't_admissao_c'
    # =========================
    if t_adm_col and _has_instrument_key(instruments_df, "t_admissao_c"):
        t_sd_guess = None
        cand = []
        for c in df.columns:
            cl = str(c).lower()
