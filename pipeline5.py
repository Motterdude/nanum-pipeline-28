...
def build_final_table(ponto: pd.DataFrame, lhv: pd.DataFrame, mappings: dict, instruments: dict) -> pd.DataFrame:
    ...
    # proteção: consumo não pode apontar para potência
    if F_mean == P_mean:
        fallback = "Consumo_kg_h_mean_of_windows"
        if fallback in df.columns:
            print(f"[WARN] fuel_kgh.col_mean estava igual a power_kw ({F_mean}). Usando fallback '{fallback}'.")
            F_mean = fallback
        else:
            raise KeyError(f"fuel_kgh.col_mean está igual a power_kw ({F_mean}) e não existe '{fallback}' no dataframe.")
    ...
    # incerteza combinada direta (pra conferir “na mão”)
    uP = df["uc_P_kw"]
    uF = df["uc_Consumo_kg_h"]
    uL = df["uB_LHV_kJ_kg"]  # só tem tipo B aqui
    ...
    # (mantém também A/B separado, como você já vinha usando)
    ...
    required_cols = [
        ...
        "uB_LHV_kJ_kg",
        "uc_n_th_check", "U_n_th_check", "U_n_th_check_pct",
