...
def build_final_table(ponto: pd.DataFrame, lhv: pd.DataFrame, mappings: dict, instruments: dict) -> pd.DataFrame:
    ...
    # garantir consumo certo
    if F_mean == P_mean:
        F_mean = "Consumo_kg_h_mean_of_windows"

    P_sd = resolve_col(df, mappings["power_kw"].get("sd", "")) if mappings["power_kw"].get("sd", "") else ""
    F_sd = resolve_col(df, mappings["fuel_kgh"].get("sd", "")) if mappings["fuel_kgh"].get("sd", "") else ""

    # defaults corretos se vierem errados do excel
    if "Consumo_kg_h_sd_of_windows" in df.columns:
        F_sd = "Consumo_kg_h_sd_of_windows"
    ...
    # incerteza combinada da eficiência (em %)
...
def make_plots(out_df: pd.DataFrame) -> None:
    """
    Gera:
      - out/nth_vs_power_H06.png
      - out/nth_vs_power_H25.png
      - out/nth_vs_power_H35.png
      - out/nth_vs_power_all.png
    com barras de incerteza (U_n_th_pct).
    """
    OUT_DIR.mkdir(exist_ok=True)

    # Seleciona colunas necessárias
    need = {"Load_kW", "H2O_pct", "n_th_pct"}
    missing = need - set(out_df.columns)
    if missing:
        print(f"[WARN] Não gerei gráficos: faltam colunas {missing}")
        return

    yerr_col = "U_n_th_pct" if "U_n_th_pct" in out_df.columns else None
    if yerr_col is None:
        print("[WARN] Não gerei gráficos: não encontrei U_n_th_pct (barras de incerteza).")
        return

    # Configuração dos eixos
    x_ticks = list(range(0, 56, 5))
    y_ticks = list(range(0, 43, 2))

    # Curvas desejadas
    targets = [6, 25, 35]

    # 3 gráficos separados
    for h in targets:
        d = out_df[out_df["H2O_pct"].astype("Int64") == h].copy()
        d["Load_kW"] = pd.to_numeric(d["Load_kW"], errors="coerce")
        d = d.dropna(subset=["Load_kW", "n_th_pct", yerr_col]).sort_values("Load_kW")
        if d.empty:
            print(f"[WARN] Sem dados para H2O_pct={h}.")
            continue

        plt.figure()
        plt.errorbar(d["Load_kW"], d["n_th_pct"], yerr=d[yerr_col], fmt="o-", capsize=4)
        plt.xlim(0, 55)
        plt.ylim(0, 42)
        plt.xticks(x_ticks)
        plt.yticks(y_ticks)
        ...
        plt.title(f"n_th vs Power (H2O={h}%)")
        plt.grid(True, which="both", linestyle="--", linewidth=0.5)
        outpath = OUT_DIR / f"nth_vs_power_H{h:02d}.png"
        plt.tight_layout()
    ...
    for h in targets:
        ...
        if d.empty:
            continue
        any_curve = True
        plt.errorbar(d["Load_kW"], d["n_th_pct"], yerr=d[yerr_col], fmt="o-", capsize=3, label=f"H2O={h}%")
    ...
    if any_curve:
        ...
        plt.xlabel("Power (kW)")
        plt.ylabel("Thermal efficiency (%)")
        plt.title("n_th vs Power (with uncertainty)")
        plt.grid(True, which="both", linestyle="--", linewidth=0.5)
        plt.legend()
        outpath = OUT_DIR / "nth_vs_power_all.png"
        plt.tight_layout()
        plt.savefig(outpath, dpi=200)
        plt.close()
        print(f"[OK] Salvei {outpath}")
        ...
        plt.close()
        print("[WARN] Não gerei gráfico combinado: sem dados.")
