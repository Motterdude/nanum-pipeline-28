...
def make_plots_all(out_df: pd.DataFrame) -> None:
    ...
    # TODOS os Kibox params (igual AI_50: sem barras)
    kibox_cols = [c for c in out_df.columns if str(c).startswith("KIBOX_")]

    # filtra colunas que são “índice/ciclo” e que só poluem
    drop_like = {"kibox_cycle", "kibox_cycle_number", "kibox_cyclenumber"}
    filtered = []
    for c in kibox_cols:
        cl = c.lower().replace(" ", "_")
        if "cycle" in cl and ("avg" not in cl and "std" not in cl and "apmax" not in cl and "imep" not in cl):
            # remove “Cycle number”, “CYCLE”, etc.
            continue
        if c in {"KIBOX_N_files"}:
            continue
        filtered.append(c)

    if not filtered:
        print("[WARN] Não achei colunas Kibox para plotar.")
        return
