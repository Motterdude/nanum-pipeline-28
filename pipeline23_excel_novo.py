...
def _find_first_col_by_substrings(df: pd.DataFrame, substrings: List[str]) -> Optional[str]:
    """
    Finds first column that contains all substrings (case-insensitive).
    Example: substrings=["t", "admiss"] will match "T_ADMISSÃO_mean_mean_of_windows".
    """
...
def _find_kibox_col_by_tokens(df: pd.DataFrame, tokens: List[str]) -> Optional[str]:
    """
    Find a KIBOX_ column by token containment, robust to underscores/spaces.
    Example tokens=["ai", "90"] matches: KIBOX_AI90, KIBOX_AI_90, KIBOX_Ai90_1, etc.
    """
...
def _round_half_up_to_resolution(x: pd.Series, res: float) -> pd.Series:
    """
    Round to a given resolution using half-up rule (not bankers).
    res must be > 0.
    """
    ...
    if res <= 0:
        return s
    q = s / res
    # half-up for positive and negative
    pos = q.where(q >= 0)
    neg = q.where(q < 0)
    ...
    out = q.copy()
    out = out.where(out.isna(), np.nan)  # keep NaN
    out = out.where(False, out)  # placeholder; will overwrite below
...
# =========================
# Psychrometrics helpers (RH% + T(C) + P(kPa abs)) -> abs humidity, w, cp_mix
# =========================
def _psat_water_pa_magnus(T_C: pd.Series) -> pd.Series:
    """
    Saturation vapor pressure of water using Magnus-Tetens approximation.
    Output in Pa. Valid for typical HVAC/engine intake ranges.
    """
    T = pd.to_numeric(T_C, errors="coerce")
    # e_s (hPa) = 6.112 * exp(17.62*T / (243.12+T))
...
def _humidity_ratio_w_from_rh(T_C: pd.Series, RH_pct: pd.Series, P_kPa_abs: pd.Series) -> pd.Series:
    """
    Humidity ratio w [kg_vapor / kg_dry_air] from RH%, T(C), total pressure P(kPa abs).
    """
    T = pd.to_numeric(T_C, errors="coerce")
...
def _absolute_humidity_g_m3(T_C: pd.Series, RH_pct: pd.Series) -> pd.Series:
    """
    Absolute humidity of water vapor in air [g/m^3] using pv and ideal gas relation:
      rho_v = pv / (R_v * T_K)
    where pv = RH * psat(T).
    """
    T = pd.to_numeric(T_C, errors="coerce")
...
def _cp_air_dry_kj_kgk(T_C: pd.Series) -> pd.Series:
    """
    Simple cp(T) for dry air in kJ/(kg*K).
    Keep it stable/robust: weak temperature dependence only.
    """
    T = pd.to_numeric(T_C, errors="coerce")
    # baseline 1.005 at 25C with a mild slope (~0.0001 per °C)
    return 1.005 + 0.0001 * (T - 25.0)


def _cp_moist_air_kj_kgk(T_C: pd.Series, RH_pct: pd.Series, P_kPa_abs: pd.Series) -> pd.Series:
    """
    cp of humid air mixture (dry air + water vapor), mass-fraction weighted.
    """
    w = _humidity_ratio_w_from_rh(T_C, RH_pct, P_kPa_abs)
    yv = w / (1.0 + w)  # mass fraction of vapor
...
def _choose_config_path() -> Path:
    """
    Prefer rev2 configs if present, else fallback to legacy.
    """
    candidates = [
...
def load_config_excel() -> Tuple[dict, pd.DataFrame, pd.DataFrame]:
    """
    Returns:
      - mappings: dict key -> {"mean": col_mean, "sd": col_sd}
      - instruments_df: rev2 instrument specs (may contain multiple rows per key to stack)
      - reporting_df: reporting rounding rules (may be empty)
    """
    p = _choose_config_path()

    # ---- Mappings (same idea as legacy; now may include extra cols)
    ...
    # ---- Instruments (rev2 preferred, legacy supported)
    ...
    if not is_rev2 and not ins.empty:
        # legacy -> rev2 conversion
        # legacy columns: key, model, dist, percent, digits, lsd, abs, resolution
...
# =========================
# Instruments rev2: uB computation (stack components)
# =========================
def _default_instruments_for_key_if_missing(instruments_df: pd.DataFrame, key_norm: str) -> pd.DataFrame:
    """
    Adds conservative defaults for specific keys if no instrument rows exist.
    Currently: T_ADMISSAO_C assumed Type K class 2 thermocouple + NI9213 CJC max.
    """
    has = instruments_df["key_norm"].eq(key_norm).any() if not instruments_df.empty else False
    if has:
        return instruments_df

    if key_norm == "t_admissao_c":
        rows = [
            {
                "key": "T_ADMISSAO_C",
                "component": "TC_K_Class2_low",
                "dist": "rect",
                "range_min": -40.0,
                "range_max": 333.0,
                "acc_abs": 2.5,
                ...
                "lsd": 0.0,
                "resolution": 0.0,
                "source": "IEC 60584 class 2 (common 'standard')",
                "notes": "±2.5°C for -40..333°C",
                "key_norm": "t_admissao_c",
            },
            {
                "key": "T_ADMISSAO_C",
                "component": "TC_K_Class2_high",
                "dist": "rect",
                "range_min": 333.0,
                "range_max": 1200.0,
                "acc_abs": 0.0,
                ...
                "resolution": 0.0,
                "source": "IEC 60584 class 2 (common 'standard')",
                "notes": "±0.75% of reading for 333..1200°C",
                "key_norm": "t_admissao_c",
            },
            {
                "key": "T_ADMISSAO_C",
                "component": "NI9213_CJC_max",
                "dist": "rect",
                "range_min": pd.NA,
                "range_max": pd.NA,
                "acc_abs": 2.1,
                ...
                "lsd": 0.0,
                "resolution": 0.0,
                "source": "NI 9213 CJC max (conservative placeholder)",
                "notes": "Use max CJC as first test; refine with NI 9213 Type K total error later if desired",
                "key_norm": "t_admissao_c",
            },
        ]
        add = pd.DataFrame(rows)
        base = instruments_df.copy()
        if base.empty:
            base = add.copy()
        else:
...
def uB_from_instruments_rev2(x: pd.Series, key_norm: str, instruments_df: pd.DataFrame) -> pd.Series:
    """
    Computes Type B standard uncertainty for a logical key using rev2 instruments table:
      - stacks multiple components via RSS
      - applies optional range_min/range_max
      - supports acc_abs, acc_pct, digits*lsd, resolution
      - dist: rect -> limit/sqrt(3), normal -> limit (already 1σ)
    """
    if instruments_df is None:
        return pd.Series([pd.NA] * len(x), index=x.index)

    ins = _default_instruments_for_key_if_missing(instruments_df, key_norm)
    if ins.empty:
        return pd.Series([pd.NA] * len(x), index=x.index)

    rows = ins[ins["key_norm"].eq(key_norm)].copy()
    if rows.empty:
        return pd.Series([pd.NA] * len(x), index=x.index)

    xv = pd.to_numeric(x, errors="coerce")

    # accumulate u_component^2
...
# =========================
def compute_trechos_stats(lv_raw: pd.DataFrame, instruments_legacy_unused: dict) -> pd.DataFrame:
    ...
    # consumption Type B from balance resolution (kept as before)
    res_kg = 0.001
...
# Final table
# =========================
def _add_uncertainty_workflow_cols(
    df: pd.DataFrame,
    key_norm: str,
    x: pd.Series,
    sd: Optional[pd.Series],
    N: pd.Series,
    ...
    """
    Standard workflow:
      - Equipment (Type B): uB_*
      - Measurement (Type A): uA_*
      - Combined: uc_*
      - Expanded: U_*
    prefix should be a clean identifier, e.g. "P_kw", "Consumo_kg_h", "T_ADMISSAO_C", "LHV_kJ_kg".
    """
    out = df.copy()

    # Type A
    if sd is not None:
        out[f"uA_{prefix}"] = pd.to_numeric(sd, errors="coerce") / (pd.to_numeric(N, errors="coerce") ** 0.5)
    else:
        out[f"uA_{prefix}"] = pd.NA

    # Type B
    out[f"uB_{prefix}"] = uB_from_instruments_rev2(x, key_norm=key_norm, instruments_df=instruments_df)
...
def _apply_reporting_rounding(df: pd.DataFrame, mappings: dict, reporting_df: pd.DataFrame) -> pd.DataFrame:
    """
    Adds *new* columns with suffix '_report' for mapped mean columns, based on Reporting_Rounding.
    Does NOT modify existing columns and does NOT change row count.
    """
    ...
    for _, r in reporting_df.iterrows():
        ...
        if rule == "round_half_up":
            ...
            # fallback: numpy round to nearest resolution
...
def build_final_table(
    ...
    # ======================================================
    # REMOVE Kibox buggy MBF 10-90 columns from DATA (and thus Excel/plots)
    # ======================================================
    ...
    # ======================================================
    # Compute MFB_10_90 = KIBOX_AI90 - KIBOX_AI10 (robust matching)
    # Adds ONE new column, does NOT change row count.
    # ======================================================
    ...
    # =========================
    # Core uncertainties: power, fuel, LHV, n_th
    # Standardize: equipment (uB), measurement (uA), combined (uc), expanded (U)
    # =========================
    ...
    P_sd = resolve_col(df, mappings["power_kw"]["sd"]) if mappings["power_kw"].get("sd") else None
    F_sd = resolve_col(df, mappings["fuel_kgh"]["sd"]) if mappings["fuel_kgh"].get("sd") else None

    N = pd.to_numeric(df["N_trechos_validos"], errors="coerce")

    # Power workflow
    df = _add_uncertainty_workflow_cols(
        df=df,
        key_norm="power_kw",
        x=pd.to_numeric(df[P_mean], errors="coerce"),
        sd=pd.to_numeric(df[P_sd], errors="coerce") if P_sd else None,
        N=N,
    ...
    df = _add_uncertainty_workflow_cols(
        df=df,
        key_norm="fuel_kgh",
        x=pd.to_numeric(df[F_mean], errors="coerce"),
        sd=pd.to_numeric(df[F_sd], errors="coerce") if F_sd else None,
        N=N,
        instruments_df=instruments_df,
        prefix="Consumo_kg_h",
    )

    # LHV workflow (typically only Type B; Type A NA)
    df = _add_uncertainty_workflow_cols(
        df=df,
        key_norm="lhv_kj_kg",
        x=pd.to_numeric(df[L_col], errors="coerce"),
        sd=None,
        N=N,
        instruments_df=instruments_df,
        prefix="LHV_kJ_kg",
    ...
    # Keep legacy names for compatibility with existing plots and downstream:
    # (do NOT remove/rename anything existing; just create as needed)
    df["uA_P_kw"] = df["uA_P_kw"]  # already created by workflow
    df["uB_P_kw"] = df["uB_P_kw"]
    df["uc_P_kw"] = df["uc_P_kw"]
    df["U_P_kw"] = df["U_P_kw"]

    df["uA_Consumo_kg_h"] = df["uA_Consumo_kg_h"]
    df["uB_Consumo_kg_h"] = pd.to_numeric(df.get("uB_Consumo_kg_h_mean_of_windows", pd.NA), errors="coerce")
    # If we computed uB_Consumo_kg_h from instruments_df above, keep it in uB_Consumo_kg_h_instrument
    # while preserving the existing balance-based uB_Consumo_kg_h that you already use.
    df["uB_Consumo_kg_h_instrument"] = df["uB_Consumo_kg_h"]
    df["uB_Consumo_kg_h"] = pd.to_numeric(df["uB_Consumo_kg_h"], errors="coerce")

    # Recompute combined/expanded for fuel using the existing balance-based uB (as your pipeline expects)
    ...
    # For power, keep the workflow (instrument-based) as the primary uB
    df["U_P_kw"] = df["U_P_kw"]

    # LHV Type B (keep existing naming used in n_th)
    df["uB_LHV_kJ_kg"] = df["uB_LHV_kJ_kg"]
    df["uc_LHV_kJ_kg"] = df["uc_LHV_kJ_kg"]
    df["U_LHV_kJ_kg"] = df["U_LHV_kJ_kg"]
    ...
    rel_uc = (
        (pd.to_numeric(df["uc_P_kw"], errors="coerce") / PkW) ** 2
        + (pd.to_numeric(df["uc_Consumo_kg_h"], errors="coerce") / Fkgh) ** 2
        + (pd.to_numeric(df["uB_LHV_kJ_kg"], errors="coerce") / LHVv) ** 2
    ) ** 0.5
    df["uc_n_th"] = df["n_th"] * rel_uc
    df["U_n_th"] = K_COVERAGE * df["uc_n_th"]
    df["U_n_th_pct"] = df["U_n_th"] * 100.0

    # airflow channels (lambda=1.0 for now)
    lambda_col = None
    ...
    # Keep row count. Do not rename existing columns.
    # =========================
    t_cil_cols = [
    ...
    if "Air_kg_h" in df.columns and t_adm_col and "T_E_CIL_AVG_mean_of_windows" in df.columns:
        mdot_air = pd.to_numeric(df["Air_kg_h"], errors="coerce") / 3600.0  # kg/s
    ...
    # =========================
    # NEW: Temperature uncertainty workflow for T_ADMISSÃO_mean_mean_of_windows
    # Key: T_ADMISSAO_C (thermocouple Type K + NI9213 CJC defaults if missing in config)
    # =========================
    # We treat the mean column found by substring as the target; you can also map explicitly via Mappings.
    if t_adm_col:
        # Ensure key exists in mappings for reporting; for uncertainty we compute directly here to test.
        # Type A: use corresponding SD column if present; else NA.
        t_sd_guess = None
        # prefer the known pattern: T_ADMISSÃO_sd_of_windows or *_sd_of_windows
        cand = []
        for c in df.columns:
            cl = str(c).lower()
            if "t_admiss" in cl and ("sd_of_windows" in cl or "_sd_" in cl):
                cand.append(c)
        if cand:
            t_sd_guess = cand[0]
        ...
        df = _add_uncertainty_workflow_cols(
            df=df,
            key_norm="t_admissao_c",
            x=pd.to_numeric(df[t_adm_col], errors="coerce"),
            sd=pd.to_numeric(df[t_sd_guess], errors="coerce") if t_sd_guess else None,
            N=N,
            instruments_df=instruments_df,
            prefix="T_ADMISSAO_C",
        )
    ...
    # =========================
    # Reporting rounding (rev2): adds *_report columns for mapped mean columns
    # =========================
...
def plot_all_fuels_xy(
    ...
    """
    Same "All fuels" logic but with arbitrary x-axis (no fixed power ticks).
    """
    PLOTS_DIR.mkdir(parents=True, exist_ok=True)
...
def _annotate_points_variants(ax, x: np.ndarray, y: np.ndarray, variant: str) -> None:
    """
    Adds value labels (2 decimals) to each point for the delta-T plot.
    Variants:
      - "box": text with white bbox next to point
      - "tag": small callout box with arrow to point
      - "marker": marker + label above point (no bbox)
      - "badge": semi-transparent rounded bbox centered near point
    """
...
def plot_all_fuels_with_value_labels(
    ...
    """
    "All fuels" plot like plot_all_fuels (no yerr), but adds per-point y value labels (2 decimals)
    using a few render styles, to help read exact delta-T.
    Does NOT change df or row count.
    """
    PLOTS_DIR.mkdir(parents=True, exist_ok=True)
    x_ticks = list(range(0, 56, 5))
...
def make_plots_all(out_df: pd.DataFrame) -> None:
    ...
    # MFB_10_90 plot vs Power (traditional)
    ...
    # NEW: Delta-T (engineered) vs Power
    if "DT_ADMISSAO_TO_T_E_CIL_AVG_C" in out_df.columns:
        ...
        # NEW: Delta-T with point value labels (render variants to choose)
    ...
    if "Q_EVAP_NET_kW" in out_df.columns:
        ...
        # Q_EVAP_NET vs air mass flow (kg/h)
        if "Air_kg_h" in out_df.columns:
            plot_all_fuels_xy(
                out_df,
                "Air_kg_h",
    ...
    # requested RAW pressure / temperature plots (all fuels) — no yerr
    raw_cols_requested = [
...
def main() -> None:
    ...
    trechos = compute_trechos_stats(lv_raw, instruments_legacy_unused={})
