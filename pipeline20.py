...
def build_final_table(ponto: pd.DataFrame, lhv: pd.DataFrame, kibox_agg: pd.DataFrame, mappings: dict, instruments: dict) -> pd.DataFrame:
    ...
    # as requested: "Tire elas do gráfico e dos dados enviados..."
    # ======================================================
    ...
    # ======================================================
    # NEW: Compute MFB_10_90 = KIBOX_AI90 - KIBOX_AI10 (robust matching)
    ...
    # =========================
    # NEW: T_E_CIL_AVG_mean_of_windows (avg of all T_S_CIL_* mean_of_windows channels)
    ...
    # =========================
    # NEW: absolute humidity + cp mix + Q_EVAP_NET (kW)
    # Uses:
    #   T_ADMISSÃO (post IC, pre plenum)  -> air inlet temperature
    #   P_COLETOR (kPa abs)              -> total pressure for psychrometrics (w)
    #   UMIDADE (RH %)                   -> relative humidity
    #   Air_kg_h                         -> mass flow proxy
    #   T_E_CIL_AVG_mean_of_windows      -> runner outlet temperature proxy
    # =========================
...
def make_plots_all(out_df: pd.DataFrame) -> None:
    ...
    # NEW: MFB_10_90 plot vs Power (traditional)
    ...
    if "Q_EVAP_NET_kW" in out_df.columns:
        ...
        # additional: Q_EVAP_NET vs air mass flow (kg/h)
        if "Air_kg_h" in out_df.columns:
            plot_all_fuels_xy(
                out_df,
                "Air_kg_h",
        ...
        # additional: Q_EVAP_NET vs P_COLETOR (kPa)
    ...
    # NEW: requested RAW pressure / temperature plots (all fuels) — no yerr
    raw_cols_requested = [
        ...
        "T_ADMISSÃO_mean_mean_of_windows",  # NEW: explicitly requested
    ...
    for c in raw_cols_requested:
        ...
        # y-axis label heuristic
        yl = "Pressure (kPa)" if c.startswith("P_") else "Temperature (°C)"

        # graph-only rename: T_S_CIL_* -> T_E_CIL_* in title/filename/label
        disp_c = c
    ...
    # Note: buggy MBF_10_90 columns were removed from the dataframe upstream, so they won't appear here.
...
# (copiado do seu código, sem alteração)
# =========================
