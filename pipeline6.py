...
def build_final_table(ponto: pd.DataFrame, lhv: pd.DataFrame, mappings: dict, instruments: dict) -> pd.DataFrame:
    ...
    # consumo não pode apontar para potência
    if F_mean == P_mean:
        fallback = "Consumo_kg_h_mean_of_windows"
        print(f"[WARN] fuel_kgh.col_mean estava igual a power_kw ({F_mean}). Usando '{fallback}'.")
        if fallback not in df.columns:
            raise KeyError(f"Não existe '{fallback}' no dataframe.")
        F_mean = fallback

    # --- SDs: corrigir automaticamente se Mappings estiver errado ---
    P_sd_name = mappings["power_kw"].get("sd", "")
    P_sd = resolve_col(df, P_sd_name) if P_sd_name else ""
    if P_sd and not P_sd.endswith("_sd_of_windows"):
        # tenta derivar do mean
        derived = P_mean.replace("_mean_mean_of_windows", "_mean_sd_of_windows")
        if derived in df.columns:
            print(f"[WARN] power_kw.col_sd='{P_sd}' não parece sd_of_windows. Usando derivado: '{derived}'.")
            P_sd = derived

    F_sd_name = mappings["fuel_kgh"].get("sd", "")
    F_sd = resolve_col(df, F_sd_name) if F_sd_name else ""
    if F_sd and F_sd != "Consumo_kg_h_sd_of_windows":
        if "Consumo_kg_h_sd_of_windows" in df.columns:
            print(f"[WARN] fuel_kgh.col_sd='{F_sd}' não é 'Consumo_kg_h_sd_of_windows'. Usando fallback correto.")
            F_sd = "Consumo_kg_h_sd_of_windows"

    print(f"[INFO] Colunas: P_mean='{P_mean}', P_sd='{P_sd}', F_mean='{F_mean}', F_sd='{F_sd}', LHV='{L_col}'")
    ...
    # incerteza combinada da eficiência (direta)
    uP = df["uc_P_kw"]
    uF = df["uc_Consumo_kg_h"]
    uL = df["uB_LHV_kJ_kg"]
    rel_uc = ((uP / PkW) ** 2 + (uF / Fkgh) ** 2 + (uL / LHVv) ** 2) ** 0.5
    df["uc_n_th_check"] = df["n_th"] * rel_uc
    df["U_n_th_check"] = K_COVERAGE * df["uc_n_th_check"]
    df["U_n_th_check_pct"] = df["U_n_th_check"] * 100.0

    # também mantém A/B separado (opcional)
